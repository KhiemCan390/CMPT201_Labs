...
Ans: The client creates a AF_INET (IPv4) with SOCK_STREAM, so the protocol here is TCP. It connects
to the server at 127.0.0.01:8001, and then use write() to send data.
2. What data is the client sending to the server?
Ans: It sends 5 messages: "Hello", "Apple", "Car", "Green", "Dog"
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
Ans: run_acceptor() takes an input which is a pointer to a struct acceptor_args. This strucutres
contains an atomic boolean "run", a pointer to a structure list_handle "list_handle", and a lock
to protect that list.
2. How are received messages stored?
Ans: Each client thread creates a list_node. The message is copied into a buffer, which is then
copied in to the list_node's data. The node then is added to the shared list.
3. What does `main()` do with the received messages?
Ans: First, it creates an acceptor thread. Then, main() waits until enough message are received.
After that, it waits until the acceptor thread finishes, and collect all messages (printing each
message and free their data buffer).
